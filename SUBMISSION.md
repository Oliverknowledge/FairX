...
- Release: tag [`submission-final`](https://github.com/Oliverknowledge/FairX/releases/tag/submission-final) — the exact deployed commit is displayed live on `/proof`, read from Vercel's `VERCEL_GIT_COMMIT_SHA`, so it is always the authoritative answer to "what code is this?"
...
Release tag: submission-final
The exact deployed commit is shown live on https://fair-x-psi.vercel.app/proof
```
