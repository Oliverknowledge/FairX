...
const STEPS: Step[] = [
  {
    ...
    copy: "This canonical flow uses an honestly labelled guided TxLINE-shaped payload. Its normalized hash is the exact value recorded in the devnet market account.",
    tone: "blue",
    provenance: true,
    fields: [["Source mode", "Guided scenario"], ["Event", "GOAL · seq 2"], ["Fixture", "ENG-FRA-2026-QF"], ["Normalized hash", yes.sourceEventHash]],
  },
  {
    ...
    tone: "amber",
    fields: [["materialSeq", "2"], ["pricedAtSeq", "1"], ["Displayed YES", "40¢"], ["Fair YES", "63¢"], ["Condition", "materialSeq 2 > pricedAtSeq 1"]],
  ...
  },
  {
    ...
    copy: "The order freezes the 40¢ quote and enters its OrderEscrow PDA. Fair YES is already 63¢, so the order carries +23¢ of stale positive edge.",
    tone: "red",
    fields: [["Side", "YES"], ["Observed price", "40¢"], ["Fair side price", "63¢"], ["Edge", "+23¢"], ["Order escrow PDA", yes.orderPda]],
  ...
  },
  {
    ...
    tone: "red",
    fields: [["Verdict", "VOIDED_REFUNDED"], ["Destination", "REFUNDED_TO_TRADER"], ["Market PDA", yes.marketPda], ["Order PDA", yes.orderPda], ["Event hash", yes.sourceEventHash]],
  ...
  },
  {
    ...
    tone: "green",
    tamper: true,
    fields: [["Integrity", "VERIFIED"], ["Transaction count", "4"], ["Destination", "REFUNDED_TO_TRADER"], ["Tamper test", "Change a sealed event hash"]],
    links: [{ label: "Open canonical receipt verifier", href: proofData.receipt.verifierHref }],
  },
  {
    title: "The opposite safe trade finalizes to ProtocolVault.",
    copy: "NO has −23¢ edge, so it gains nothing from the stale quote. LineGuard allows it and moves the stake to ProtocolVault. It blocked only the exploitative side.",
    tone: "blue",
    fields: [["Side", "NO"], ["Observed / fair", "60¢ / 37¢"], ["Edge", "−23¢"], ["Verdict", "STALE_ALLOWED_NO_EDGE"], ["Destination", "FINALIZED_TO_VAULT"], ["Vault PDA", proofData.vault.pda]],
    links: [{ label: "Open NO finalization transaction", href: no.txs[3].explorerUrl }, { label: "Open ProtocolVault", href: proofData.vault.explorerUrl }],
  },
];

const SUMMARY = `FairX / LineGuard canonical devnet proof\nProgram: ${proofData.program.id}\nEvent: guided TxLINE-shaped GOAL seq 2, hash ${yes.sourceEventHash}\nYES: +23¢ edge → VOIDED_REFUNDED → trader\nNO: −23¢ edge → STALE_ALLOWED_NO_EDGE → ProtocolVault ${proofData.vault.pda}\nLineGuard did not freeze the market. It blocked only the exploitative side.`;
    ...
    <section className="overflow-hidden rounded-2xl border border-(--border) bg-white shadow-[0_16px_55px_rgba(15,23,42,0.07)]">
      ...
      <div className="grid min-h-[470px] lg:grid-cols-[minmax(0,1.2fr)_390px]">
        <div className="flex min-w-0 flex-col p-5 sm:p-7">
          ...
          {step.provenance && <div className="mt-5 max-w-2xl"><TxLineProvenance compact mode="guided" endpoint="No network — guided TxLINE-shaped payload" fixtureId="ENG-FRA-2026-QF" eventType="GOAL" sequence={2} receivedAt={1_783_615_318_241} rawEventHash={proofData.receipt.receipt.rawEventHash} normalizedEventHash={yes.sourceEventHash} proofState="Normalized hash committed on Solana devnet" /></div>}
        ...
        <aside className="border-t border-(--border) bg-[#f8fafc] p-4 sm:p-5 lg:border-l lg:border-t-0">
          ...
          <p className="mt-5 border-t border-(--border) pt-3 text-[9.5px] leading-relaxed text-(--ink-3)">Canonical verified proof remains available if fresh devnet execution is unavailable. This sequence never labels guided evidence as live TxLINE.</p>
        </aside>
      </div>
    </section>
  );
