    ...
    <FairXShell compact>
      <div className="mx-auto max-w-[1120px]">
        <header className="grid gap-8 border-b border-(--border) pb-10 pt-3 lg:grid-cols-[1fr_.8fr] lg:items-end">
          <div><p className="inline-flex items-center gap-2 text-[10px] font-bold uppercase tracking-[.1em] text-(--blue)"><Code2 className="h-4 w-4" />For prediction-market operators</p><h1 className="mt-4 max-w-3xl text-[40px] font-extrabold leading-[.99] tracking-[-.055em] sm:text-[60px]">Integrate with confidence.</h1><p className="mt-5 max-w-2xl text-[14px] leading-7 text-(--ink-2)">Run every decision and validation branch before touching production. The frozen V4 sequence rule is exact: an order <strong>behind</strong> the required event sequence returns its principal, an order <strong>level</strong> with it is accepted, and an order <strong>ahead</strong> of it is rejected as <strong>FUTURE_SEQUENCE</strong>.</p></div>
        ...
        <div className="mt-8"><IntegrationKitDemo preQuote={CANONICAL_PRE_GOAL_QUOTE} postQuote={CANONICAL_POST_GOAL_QUOTE} /></div>

        <section className="mt-5 overflow-hidden rounded-2xl border border-(--border) bg-white" aria-labelledby="operator-reality"><div className="border-b border-(--border) p-5"><p className="section-label text-(--blue)">Operator reality</p><h2 id="operator-reality" className="mt-2 text-[22px] font-extrabold">What this reference proves—and what it does not.</h2></div><dl className="grid gap-px bg-(--border) sm:grid-cols-2 lg:grid-cols-4"><RealityFact label="Network" value="Solana devnet" /><RealityFact label="Browser demo" value="Deterministic no-send adapter · 0 transactions" /><RealityFact label="On-chain path" value="Deployed Vault V4 · 24 recorded lifecycle transactions" /><RealityFact label="Order outcomes" value="ACCEPTED · STALE_SEQUENCE_RETURNED" /><RealityFact label="Canonical scope" value="The deployed program is pinned to the recorded France–Morocco fixture. This lab exercises the reusable policy interface, not a second on-chain fixture." /><RealityFact label="Fees" value="Network transaction fees remain separate from principal" /><RealityFact label="External audit" value="None · unaudited hackathon prototype" /><RealityFact label="Custody" value="Consumer custody and wallet flows are outside this reference" /><RealityFact label="Authority" value="Configured roles · 2-of-3 resolution · single upgrade authority" /></dl></section>
