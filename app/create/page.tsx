...
import type { FairXMarketType, FairXSource, MaterialityRules } from "@/lib/markets/fairx";
    ...
    <FairXShell>
      <div className="mx-auto max-w-[1120px]">
        <header className="mb-5 flex flex-col gap-3 border-b border-(--border) pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="section-label">Creator market builder</p>
            <h1 className="mt-2 text-[29px] font-bold leading-none tracking-[-0.055em] text-(--ink)">Create a protected sandbox market.</h1>
            <p className="mt-3 max-w-2xl text-[12px] leading-relaxed text-(--ink-2)">Define a sandbox market with LineGuard protection — its fair-price tolerance and which source events are material enough to open a stale window. It can then be initialized on devnet.</p>
          </div>
        ...
        <form onSubmit={submit} className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_330px]">
          <div className="space-y-4">
            <section className="card p-4 sm:p-5">
              <p className="section-label">Market definition</p>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Field label="Settlement source" error={errors.source}>
                  <select value={form.source} onChange={(event) => change("source", event.target.value as FairXSource)} className={inputClass(errors.source)}>
                    <option value="live">TxLINE live</option>
                    <option value="captured">Captured TxLINE replay</option>
                    <option value="demo">Guided scenario</option>
                  </select>
                </Field>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <Field label="LineGuard tolerance" error={errors.tolerance} hint="A 0.02 tolerance equals 2¢.">
            ...
            <section className="rounded-lg border border-[#f0d39a] bg-(--amber-bg) p-4 text-[10.5px] leading-relaxed text-[#9b650d]">
              <div className="flex gap-2"><CircleAlert className="mt-0.5 h-4 w-4 shrink-0" /><p><strong>Execution boundary:</strong> this builder creates sandbox markets that run as local simulation. The canonical LineGuard YES/NO flow executes fresh transactions on devnet, and a sandbox market can be initialized on devnet — but its trading is not devnet-settled until orders are routed on-chain.</p></div>
            </section>
          </div>
          ...
          <aside className="space-y-3 lg:sticky lg:top-4 lg:self-start">
            ...
            <button type="submit" className="inline-flex h-11 w-full items-center justify-center gap-2 rounded-md bg-(--ink) px-3 text-[11px] font-bold text-white hover:bg-[#273244]"><PlusCircle className="h-4 w-4" />Create protected sandbox market <ArrowRight className="h-3.5 w-3.5" /></button>
