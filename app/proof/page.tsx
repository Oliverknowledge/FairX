    ...
    <FairXShell compact>
      <div className="mx-auto max-w-[1100px]">
        ...
        <section className="mt-10 grid gap-4 lg:grid-cols-2" aria-label="FairX trust boundary">
          ...
          <article className="rounded-2xl border border-amber-200 bg-amber-50 p-5 sm:p-6"><div className="flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-white text-amber-700"><Settings2 className="h-5 w-5" /></span><div><p className="text-[8.5px] font-bold uppercase tracking-[.09em] text-amber-700">Trusted or configurable</p><h2 className="mt-1 text-[18px] font-extrabold">What FairX does not make trustless</h2></div></div><ul className="mt-5 grid gap-2 sm:grid-cols-2"><BoundaryItem text="TxLINE supplies source evidence for fixture events, odds and final result" muted /><BoundaryItem text="The pricing authority must stay available and secure; it cannot choose an arbitrary V4 quote" muted /><BoundaryItem text="Two of three configured resolution authorities approve settlement" muted /><BoundaryItem text="The deployed program currently retains an upgrade authority" muted /><BoundaryItem text="The deployed program is pinned to the canonical fixture and is not generic across fixtures" muted /></ul></article>
        </section>

        <section className="mt-4 rounded-2xl border border-(--border) bg-white p-5 sm:p-6" aria-labelledby="canonical-scope">
          <p className="text-[8.5px] font-bold uppercase tracking-[.09em] text-(--ink-3)">Scope of the deployed reference</p>
          <h2 id="canonical-scope" className="mt-1 text-[18px] font-extrabold">Canonical V4 scope: pinned by design</h2>
          <p className="mt-3 max-w-3xl text-[11px] leading-6 text-(--ink-2)">
            The deployed reference implementation is intentionally pinned to the recorded France–Morocco fixture (<span className="font-mono">18209181</span>) and its committed TxLINE evidence. <span className="font-mono">CANONICAL_*</span> constants fix the fixture identity, participants, material-event sequences and the captured pre/post StablePrice odds, and <span className="font-mono">validate_canonical_identity</span> rejects any other market — so <span className="font-mono">initialize_market_v4</span> cannot bind a second fixture without new constants and a redeploy.
          </p>
          <p className="mt-3 max-w-3xl text-[11px] leading-6 text-(--ink-2)">
            That pinning is what makes this lifecycle tamper-evident: the recorded sequences, odds and payouts on this page cannot be re-run with different inputs. Runtime scenarios and IntegrationKit demonstrate the reusable policy interface; they are not additional on-chain fixture deployments. Removing the canonical constants in favour of arbitrary fixture configuration is the next program release, not a claim made here.
          </p>
        </section>
