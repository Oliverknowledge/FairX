...
async function main() {
  ...
  for (let byte = 7; byte <= 18; byte += 1) signerList.push(await testSigner(byte));
  const [adminSigner, operatorSigner, feedSigner, pricingSigner, resolutionASigner, resolutionBSigner, resolutionCSigner, ...traderSigners] = signerList;
  ...
  const send = async (label: string, feePayer: PublicKey, instructions: Instruction[], expectFailure = false) => {
    const feePayerSigner = signerByAddress.get(kitAddress(feePayer));
    assert(feePayerSigner, `missing local test signer for ${feePayer}`);
    const signedInstructions = instructions.map((instruction) => addSignersToInstruction(signerList, instruction));
    const message = pipe(
  ...
  await send("verify pre-goal quote", admin, [computeIx, makeIx("verify_txline_quote", verifyArgs(1, fixture.preGoalOddsValidation), [[admin, roles.writableSigner], [market, roles.writable], [new PublicKey(fixture.oddsRootPda), roles.readonly], [TXLINE_PROGRAM, roles.readonly], [quoteReceipts[0], roles.writable], system])]);
  ...
  await send("verify post-goal quote", admin, [computeIx, makeIx("verify_txline_quote", verifyArgs(2, fixture.postGoalOddsValidation), [[admin, roles.writableSigner], [market, roles.writable], [new PublicKey(fixture.oddsRootPda), roles.readonly], [TXLINE_PROGRAM, roles.readonly], [quoteReceipts[1], roles.writable], system])]);
  ...
  console.log(JSON.stringify({
    mode: "LiteSVM with signature verification enabled and deterministic public test signers",
